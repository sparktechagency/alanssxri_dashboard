...
import { useUpdateNewslettersMutation } from "../../../../redux/features/newsletters/newslettersApi";
...
const EditNewslettersModal = ({ isModalOpen, handleOk, handleCancel, newsletters }: AddModalProps) => {
    const [form] = Form.useForm();
    const [api, contextHolder] = notification.useNotification();
    const [fileList, setFileList] = useState<any[]>([]);

    const [updateNewsletters, { isLoading }] = useUpdateNewslettersMutation();

    useEffect(() => {
        if (isModalOpen && newsletters) {
            form.resetFields();
            form.setFieldsValue({
    ...
    const onFinish = (values: any) => {
        ...
        updateNewsletters({ data: formData, id: newsletters?._id })
            .unwrap()
            .then(() => {
                api.success({
                    message: "Newsletters Successfully!",
...
export default EditNewslettersModal;